...
export default function BlogList() {
  ...
  return (
    <main className="min-h-screen bg-[#f8fbff] text-[#0f172a] dark:bg-[#06131a] dark:text-white">
      <PortfolioHeader
        language={language}
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-lg leading-8 text-[#5b667b]">
                Operator-level notes on AI marketing, digital growth systems,
                performance marketing, e-commerce optimization, and real
                execution logic.
              </p>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          {filteredPosts.map((post, index) => {
            ...
            return (
              <Fragment key={post.id}>
                <article className="flex h-full flex-col overflow-hidden rounded-[1.25rem] border border-[#dce7f9] bg-white shadow-[0_18px_42px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:border-[#cadcf6] hover:shadow-[0_22px_46px_rgba(15,23,42,0.09)]">
                  {thumbnail?.url ? (
                    <img
                      src={thumbnail.url}
                      alt={thumbnail.alt[language]}
                      loading="lazy"
                      decoding="async"
                      className="aspect-video w-full bg-white object-cover"
                    />
                  ) : (
                    <div className="flex aspect-video w-full items-center justify-center bg-[#eef4ff] px-6 text-center text-sm text-[#5b667b]">
                      {thumbnail?.prompt || "Blog visual"}
                    </div>
                  )}
                  <div className="flex flex-1 flex-col p-6 pt-5">
                    ...
                    <h2 className="font-['Space_Grotesk'] text-2xl font-bold leading-tight">
                      {post.seo[language].title || post.topic}
                    </h2>
                    <p className="mt-3 min-h-[4.5rem] line-clamp-3 text-sm leading-6 text-[#5b667b]">
                      {post.seo[language].metaDescription}
                    </p>
                ...
                {(index + 1) % 6 === 0 && index + 1 < filteredPosts.length ? (
                  <BlogListAd />
                ) : null}
              </Fragment>
            );
        ...
        {!filteredPosts.length ? (
          <div className="rounded-3xl border border-dashed border-[#dce7f9] bg-white p-10 text-center text-[#5b667b]">
