...
export default function ProjectsSection() {
  ...
  const VibeProjectCard = ({ project }: { project: Project }) => {
    const description = project.description[language as keyof typeof project.description] || project.description.en;
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg hover:border-blue-200 dark:hover:border-blue-600 transition-all duration-300 overflow-hidden group h-full flex flex-col">
        ...
        <div className="flex gap-4 p-5 border-b border-gray-100 dark:border-gray-700 flex-grow items-start">
          ...
          {hasImage && (
            <div className="flex-shrink-0 bg-gradient-to-br from-gray-50 dark:from-gray-700 to-gray-100 dark:to-gray-800 rounded-lg overflow-hidden flex items-center justify-center border border-gray-200 dark:border-gray-600" style={{ width: '140px', height: '100%', minHeight: '140px' }}>
              <img
                src={project.image}
                alt={project.title}
          ...
          {hasButtons && (
            <div className="flex gap-2 w-full">
              {project.ctaButtons.map((btn, idx) => {
                const btnLabel = btn.label[language as keyof typeof btn.label] || btn.label.en;
                if (btn.type === 'video') {
                  return (
                    <button
                      key={idx}
